#!/bin/bash
#BSUB -J gssl_down_tarpfp
#BSUB -o logs/downstream_ssl_tarpfp_%J.out
#BSUB -e logs/downstream_ssl_tarpfp_%J.err
#BSUB -q gpul40s
...
#BSUB -N 
#
# Downstream Evaluation: Self-Supervised TAR+PFP
# Runs only multiclass link prediction downstream task
# Submit: bsub < scripts/hpc/downstream/downstream_ssl_tarpfp.sh
#

echo "Starting Downstream Evaluation: Self-Supervised TAR+PFP"
echo "=============================================="
...
python -m graphssl.downstream_evaluation \
    --data_root data \
    --results_root results/downstream_ssl_tarpfp_${LSB_JOBID}_$(date +%Y%m%d_%H%M%S) \
    --metapath2vec_embeddings_path pos_embedding.pt \
    --model_path results/exp_ssl_tarpfp_27272751_20251204_092313/model_self_supervised_tarpfp.pt \
